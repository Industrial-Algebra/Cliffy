...
### Removed (breaking — Phase 2 WS0, the salvage reaches tsukoshi)

- `cliffy-tsukoshi`: `GeometricCRDT`, `GeometricOperation`, `OperationType`,
  `geometricMean` (protocols/crdt), `GeometricConsensus` + consensus
  plumbing (protocols/consensus), and `GA3Lattice` (magnitude-dominance
  join) — the pre-salvage surface the Rust cutover (#313) deleted but the
  TypeScript twin kept shipping (RABBIT_HOLE_2026-09-20: the salvage
  audited modules, not products). Failure modes are the same as the Rust
  deletions: replay-merge annihilation, `nextOpId++` cross-replica
  collisions, non-semilattice join.
- tsukoshi version diverges upward: **0.4.0 → 0.5.0** (breaking removal).
  The componentwise floor (`ComponentLattice`, `latticeJoin`,
  `latticeMeet`) remains — the salvage blessed it.

### Added

- `cliffy-tsukoshi`: **`ObservationSet`** (G-Set CRDT, merge = union) with
  `scalarObservation`/`vectorObservation`/`rotorObservation` constructors,
  canonical `(participant, seq)` iteration, and Rust-compatible JSON wire
  format; projections `scalarMean`, `vectorMean`, `rotorConsensus` (Markley
  eigen-mean via a faithful cyclic-Jacobi port — `eigen.ts`).
- **Cross-runtime parity vectors** (`test-vectors/observation-set-parity.json`):
  generated by `cliffy-protocols/tests/parity_vectors.rs` (which
  drift-guards: regenerated bytes must equal checked-in bytes), consumed by
  tsukoshi's vitest with **exact-equality** assertions — the determinism
  contract ("equal sets ⇒ bit-identical projections") now spans the
  language boundary. 10 cases × merge-order permutations, including the
  fossilized annihilation oracle.
